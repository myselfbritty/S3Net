...
class make_classifier(tf.keras.Model):
    ...
    def __init__(self, num_classes=10, WEIGHT_DECAY=1e-4):
        ...
        self.resnet_model = keras.models.load_model('resnet_2.h5')
        self.conv1 = LY.Conv2D(256, 1)
        self.conv2 = LY.Conv2D(256, 1)
        self.conv3 = LY.Conv2D(256, 1)
        self.conv4 = LY.Conv2D(256, 1)

        self.maskconv1 = LY.Conv2D(128,3,padding='same',activation='relu')
        self.maskconv2 = LY.Conv2D(256, 3, padding='same', activation='relu')
        self.maskconv3 = LY.Conv2D(512, 3, padding='same', activation='relu')
        self.maskconv4 = LY.Conv2D(1024, 3, padding='same', activation='sigmoid')
    ...
    def call(self, inputs, mode='margin', training=True):
        if mode == 'margin':
            ...
            ip = inputs[0]
            mask_ip = inputs[1]
            y = tf.zeros(shape=(ip.shape[0], self.num_classes))
        ...
        mask_op = self.maskconv1(mask_ip)
        mask_op = self.maskconv2(mask_op)
        mask_op = self.maskconv3(mask_op)
        mask_op = self.maskconv4(mask_op)
